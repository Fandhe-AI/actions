...
| `skip-branch-prefixes` | - | （空） | codex レビューを skip する head branch 名の接頭辞（カンマ区切り。例 `chore/skills-update,chore/submodule-update`）。`update-external.yml` が生成する日次同期 PR のような自動生成 PR をレビュー対象から外す用途。同期 PR は上流の取り込みそのもので、指摘の修正先が取り込み元の上流リポジトリにしかないため。判定は `skip-gate` ジョブがブランチ名の接頭辞のみで行い、fork PR では起動しない。判定不能時（gate の失敗・output 欠落）はレビュー実行側へ倒す fail-closed。push できる主体はブランチ名で gate を回避できる点に注意（下記「受容済み残留リスク」参照）。判定は単純な前方一致のため、無関係なブランチ（例: `chore/skills-update2-experiment`）が偶発的に一致しないよう、接頭辞は自動生成 PR だけが使う十分にユニークな文字列にすること |
