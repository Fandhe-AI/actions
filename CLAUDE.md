...
| `rust-toolchain-setup/` | Composite Action | self-hosted runner 上での rustup 自己修復と `rust-toolchain.toml` へのツールチェーン同期（`components` で追加コンポーネントを冪等インストール。先頭で github.com スコープの stale credential（`http.*.extraheader`）を取り除いたジョブ専用の隔離コピーを `GIT_CONFIG_GLOBAL` で以降のステップにのみ適用し、runner ホストの実 global config は書き換えずに公開 repo への匿名 fetch の 401 混入を防ぐガードを既定有効で実施） |
