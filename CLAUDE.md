...
| `rust-toolchain-setup/` | Composite Action | self-hosted runner 上での rustup 自己修復と `rust-toolchain.toml` へのツールチェーン同期（`components` で追加コンポーネントを冪等インストール。先頭でグローバル git config の stale credential（`http.*.extraheader`）を除去し、公開 repo への匿名 fetch の 401 混入を防ぐガードを既定有効で実施） |
